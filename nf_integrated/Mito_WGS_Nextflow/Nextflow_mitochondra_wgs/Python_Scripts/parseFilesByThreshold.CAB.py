...
def assignNewGroups_Feb24_2025(filename):
        file = open(filename, 'r')
        outputFileName = filename[:-4] + "_withNewGroups.txt"
        outputFile = open(outputFileName, 'w')
        groupINHcnt = 0
        ...
        for line in file:
                ...
                '''
                   ...
                   mas2 <- mas %>%
                       mutate(ReClass = case_when(Class == "GH" &
                                     VAF_Tumor >= 0.97 & VAF_Germline < 0.97 & VAF_Tumor / VAF_Germline <= 3 ~ "SH",
                                   TRUE ~ Class  # Keep original classification for other cases
                       ))
                '''
                if tmaf >= 0.97 and nmaf >= 0.97:
                    group='INH'
                    groupINHcnt += 1
                elif ( tmaf >= 0.03 and nmaf < 1 ) or tmaf/nmaf > 3:
                    group='TU'
                    groupTUcnt += 1
                elif nmaf >= 0.03 and tmaf < 0.03:
                    group='GH'
        ...
        print "Group INH -", groupINHcnt
        print "Group TU -", groupTUcnt
        print "Group GH -", groupGHcnt
        print "Group SH -", groupSHcnt
        outputFile.close()
...
def main():
	...
	#assignNewGroups(filename[:-4] + "_gMafOrtMaf_greaterThanOrEqual0.03.txt")
        assignNewGroups_Feb24_2025(filename[:-4] + "_gMafOrtMaf_greaterThanOrEqual0.03.txt")
