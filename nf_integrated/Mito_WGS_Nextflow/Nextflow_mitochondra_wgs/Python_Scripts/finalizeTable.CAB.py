...
def addComplexAndPredictedImpactAndMarkComSeqErrors(filename, relpath):
    file = open(filename, 'r')
    newfile = open(filename[:-4] + '_allColumns.txt', 'w')

    #refSeqFile = open('/research/groups/cab/projects/automapper/common/wzhang42/Mitochondra_Pipeline/MT_bwa/MT.fa', 'r')
    refSeqFile = open(relpath + "/Mito_WGS_Nextflow/Nextflow_mitochondra_wgs/MT_bwa/MT.fa", 'r')
    refSeq = ''
    ...
    for line in file:
        line_num+=1
        if line_num==1:
            newfile.write('flag\tcomplex\ttypeOfSNV\tsequence\t' + line)
            continue
        line = line.rstrip()
        cols = line.split('\t')
        sample = cols[0]
...
def main():
    ...
    relpath = os.path.dirname(relpath)
    addComplexAndPredictedImpactAndMarkComSeqErrors("./all.maf.fisher_annotated_gMafOrtMaf_greaterThanOrEqual0.03_withNewGroups.txt", relpath)
